...
public class SegmentService {
  ...
  public Optional<SegmentSelectedDto> getSegmentSelectedDto(Long layerId, Long segmentId) {
    ...
    return Optional.of(SegmentSelectedMapper.toDtoForSelectedSegmentViewPage(
        layer.get(),
        segment.get(),
        segmentStateLink,
        roles,
        segmentViewRequirementDtoList,
  ...
  private List<SegmentViewRequirementDto> getSegmentViewRequirementDtos(
      List<QuestionRequiredLink> links,
      Long layerId,
      List<Layer> space,
      Long segmentId
  ) {
    ...
    return links.stream()
        .map(link -> SegmentViewRequirementMapper.toDtoForSelectedSegmentViewPage(
            link,
  ...
  private List<SegmentViewEntryPointDto> getSegmentViewEntryPointDtos(Layer layer, Long segmentId) {
    ...
    return entrypoints.stream()
        .map(entrypoint -> SegmentViewEntryPointMapper.toDtoForSelectedSegmentViewPage(
            entrypoint,
            getSegmentViewScreenDtos(layer, segmentId, entrypoint.getId())
        ))
        .toList();
  }
  ...
  private List<SegmentViewScreenDto> getSegmentViewScreenDtos(Layer layer, Long segmentId, Long entrypointId) {
    ...
    return segmentScreenEntrypointLinks.stream()
        ...
        .map(link -> SegmentViewScreenMapper.toDtoForSelectedSegmentViewPage(
            link,
            layerId,
  ...
  private List<SegmentViewQuestionDto> getSegmentViewQuestionDtos(
      Long layerId,
      SegmentScreenEntrypointLink link,
      Map<String, Question> parentLayerQuestions
  ) {
    List<Layer> space = getLayersInSpace(layerId);
    ...
    return screenQuestionLinks.stream()
        ...
        .map(questionLink -> SegmentViewQuestionMapper.toDtoForSelectedSegmentViewPage(
            questionLink.getQuestion(),
            layerId,
  ...
  public List<ScreenQuestionLink> getSQLInSpace(List<Layer> space, SegmentScreenEntrypointLink link) {
    ...
    for (Layer layer : space) {
      screenQuestionLinks.addAll(screenQuestionLinkDao.findAll(
          layer.getId(),
  ...
  private List<InfoLayerEntryPointDto> getCreateLayerEntryPointDtos(List<Layer> space, Long segmentId) {
    List<Entrypoint> entrypoints = entrypointDao.findAll();
    return entrypoints.stream()
        .map(entrypoint -> InfoLayerEntryPointMapper.toDtoForLayerCreation(
            entrypoint,
            getCreateLayerScreenDtos(space, segmentId, entrypoint.getId())
        ))
        .toList();
  }
  ...
  private List<InfoLayerScreenDto> getCreateLayerScreenDtos(List<Layer> space, Long segmentId, Long entrypointId) {
    ...
    return segmentScreenEntrypointLinks.stream()
        .sorted(Comparator.comparing(SegmentScreenEntrypointLink::getScreenPosition))
        .map(link -> InfoLayerScreenMapper.toDtoForLayerCreation(
            link,
            PlatformMapper.toDtoList(platformDao.findAll(link.getScreen().getPlatforms())),
    ...
    segmentValidateInfoDto.getEntryPoints().forEach(entrypoint -> {
      ...
      validateVisibilityMap.forEach((questionId, infoLayerScreenDtos) -> {
        if (infoLayerScreenDtos.size() > 1) {
          ...
          validateResultDto.setResult(QuestionValidateResultMapper.toDto(
              entrypoint,
              question,
              ScreenValidateResultMapper.toListDto(infoLayerScreenDtos)
          ));
      ...
      validateScreenPositionMap.forEach((position, infoLayerScreenDtos) -> {
        if (infoLayerScreenDtos.size() > 1) {
          ...
          validateResultDto.setError("Внутри точки входа " + '[' + entrypoint.getTitle() + ']' + " на позиции " + position + " располагается более " +
              "одного активного экрана.");
          validateResultDto.setErrorType(ErrorType.SCREEN_POSITION);
          validateResultDto.setResult(EntrypointValidateResultMapper.toDto(
              entrypoint,
              position,
              ScreenValidateResultMapper.toListDto(infoLayerScreenDtos)
          ));
